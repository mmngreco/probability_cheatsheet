...
\item[Independencia] - Intuitivamente, dos variables aleatorias son independientes si conocer una no te da ningún tipo de informacion sobre la otra. $X$ e $Y$ son independientes si para TODOS los valores de x e y:  \begin{center}
...
\item[Función de Cuantía (PMF)] (Sólo caso discreto) es una función que toma el valor de $x$, devuelve la probabilidad de que la variable aleatoria toma ese valor $x$. La PMF es una función con valores positivos, y $\sum_xP(X=x) = 1$
\begin{center}
$P_X(x) = P(X=x)$
\end{center}

\item[Función de Densidad (CDF)] es una función que para valores $x$, devuelve la probabilidad que una variable aleatoria toma hasta ese valor $x$.
\[F(x) = P(X \leq x)\]
...
\end{center}
Nótese que \emph{cualquier} $X$ e $Y$, $a$ y $b$ coeficientes escalares y $c$ es constante, la siguiente propieda de \textbf{Linealidad de la Esperanza} mantiene:
